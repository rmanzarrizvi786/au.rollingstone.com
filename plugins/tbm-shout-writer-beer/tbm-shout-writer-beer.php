...
<?php
  ...
  public function shout_writer_beer_init() {
    ...
    if ( isset( $_POST['submit-shout-writer-beer'] ) ) {
      ...
      // var_dump( $curl_recaptcha_data ); var_dump( $recaptcha ); exit;

      if ( ! $recaptcha->success || $recaptcha->score < 0.25) :
      ...
      $data_paypal = array(
        ...
        'image_url' => 'https://thebrag.com/media/brag-media-150x50.png',
      );
  ...
  ?>
    ...
    <div style="width: auto; max-width: 100%;" class="mt-3">
      ...
      <form action="" method="post" name="form-shout-writer-beer" class="form-shout-writer-beer" target="_blank" style="display: none;">
        <textarea name="message" class="shout-writer-beer-message form-control" placeholder="Your note to <?php echo $a['author']; ?> (optional)"></textarea>
  ...
  <?php
      ...
      if ( '' != $about_url ) :
        $html .= '<a href="' . $about_url . '" target="_blank" class="text-right bg-primary text-white badge p-2" style="font-size: 80%; background: #d32531; color: #fff; padding: .25rem; border-radius: 4px;">About this</a>';
      endif;
      $html .= '</div>
      </form>
      </div>';
