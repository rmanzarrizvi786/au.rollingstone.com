...
/**
 * Plugin Name: TBM 200 Greatest Artists 2021
...
class TBM200GreatestArtists2021
{
  ...
  public function __construct()
  {
    ...
    $this->plugin_name = 'tbm_200_greatest_artists_2021';
    $this->plugin_slug = 'tbm-200-greatest-artists-2021';
    $this->post_name = 'rolling-stones-200-greatest-australian-albums-of-all-time';
    $this->total_entries = 200;

    /* AJAX */
    add_action('wp_ajax_save_artist', [$this, 'save_artist']);

    add_action('admin_menu', function () {
      // add_menu_page('200 Greatest Artists', '200 Greatest Artists', 'edit_pages', $this->plugin_slug, [$this, 'index'], 'dashicons-playlist-audio', 17);
    });
  ...
  public function save_artist()
  {
    ...
    $check = $wpdb->get_row($wpdb->prepare("SELECT id FROM {$wpdb->prefix}200_greatest_artists_2021 WHERE position = %d", $formData['position']));
    ...
    if ($check) { // Update
      $wpdb->update(
        $wpdb->prefix . '200_greatest_artists_2021',
        $values,
        ['id' => $check->id]
      );
    } else { // Insert
