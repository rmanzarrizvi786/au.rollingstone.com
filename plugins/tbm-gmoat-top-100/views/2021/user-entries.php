...
$movie_query = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}gmoat_user_movies_2021 m WHERE m.movie_title = %s LIMIT 1", $movie_title);
        ...
        $entries_query = $wpdb->prepare("
          ...
          FROM
            {$wpdb->prefix}gmoat_entries_2021 e
              JOIN {$wpdb->prefix}gmoat_user_movies_2021 um
                ON e.id = um.entry_id
          WHERE
            um.movie_title = %s
        ", $movie_title);
