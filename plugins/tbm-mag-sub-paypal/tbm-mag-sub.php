<?php
    ...
    class TBMMagSubPaypal
    {
        ...
        public function __construct()
        {
            ...
            add_action('wp_ajax_click_button', [$this, 'click_button']);
            add_action('wp_ajax_nopriv_click_button', [$this, 'click_button']);

            add_action('wp_ajax_subscribe_success', [$this, 'subscribe_success']);
            add_action('wp_ajax_nopriv_subscribe_success', [$this, 'subscribe_success']);
...
<?php
    $query = "SELECT * FROM {$wpdb->prefix}mag_subscriptions WHERE 1=1 ";
            ...
            if ('0' != $status && in_array($status, ['init', 'subscribed'])) {
...
<?php if ($sub->subscriptionID): ?>
          ...
          <td><a href="https://sales.thebrag.media/app/v1-subscription#id=<?php echo $sub->salesforce_id; ?>/view" target="_blank"><?php echo $sub->salesforce_id; ?></a></td>
        </tr>
<?php endforeach;?>
      </tbody>
<?php endif;?>
            ...
            if (check_ajax_referer($this->plugin_name . '_nonce', 'nonce')):
                ...
                if (isset($post['coupon_code']) && '' != trim($post['coupon_code'])):
                    ...
                    if ($coupon):
                        ...
                        wp_send_json_error(['result' => 'Invalid Coupon code']);
                    endif;
                else:
                    $post['coupon_code'] = '';
                endif;
                ...
                wp_send_json_success(['plan_id' => $plan_id]);
                wp_die();
            else:
                wp_send_json_error(['result' => 'Invalid request']);
            endif;
        }
		        ...
		        <?php
                            include get_template_directory() . '/email-templates/footer.php';
                                    ...
                                    if ($status != 201) {
                                        ...
                                        wp_send_json_error(['result' => 'Error processing, please contact <a href="mailto:subscribe@thebrag.media" target="_blank">subscribe@thebrag.media</a> with the details you submitted.']);
                                    } else {
                                    ...
                                    wp_send_json_error(['result' => 'Whoops, it looks like something went wrong. Please contact <a href="mailto:subscribe@thebrag.media" target="_blank">subscribe@thebrag.media</a> with the details you submitted.']);
                                endif;
                            else:
                                wp_send_json_error(['result' => 'Invalid request']);
                            endif;
                        ...
                        protected function paypal_request($request, $fields = '')
                        {
                            ...
                            if ('oauth2/token' != $request) {
                                curl_setopt($ch, CURLOPT_HTTPHEADER, ["Authorization: Bearer {$this->paypal_access_token}", 'Content-Type: application/json']);
                            } else {
