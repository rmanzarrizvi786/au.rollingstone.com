<?php
    ...
    class TBMMagSub
    {
        ...
        {
            $this->config = include __DIR__ . '/classes/config.php';
            ...
            if ($this->is_sandbox) {
                $this->api_brag_user['api_url'] = 'https://thebrag.localhost/wp-json/tbm_users/v1/';

                // Braze
                self::$api_braze['sdk_api_key'] = '08d1f29b-c48e-4bb3-aef3-e133789a0c89';
                self::$api_braze['sdk_endpoint'] = 'sdk.iad-05.braze.com';
                self::$api_braze['api_key'] = '4bdcad2b-f354-48b5-a305-7a9d77eb356e';
            } else {
            ...
            /* Stripe - Actions */
            add_action('wp_ajax_get_stripe_public_key', [$this, 'get_stripe_public_key']);
            add_action('wp_ajax_nopriv_get_stripe_public_key', [$this, 'get_stripe_public_key']);

            add_action('wp_ajax_get_stripe_payment_intent', [$this, 'get_stripe_payment_intent']);
            add_action('wp_ajax_nopriv_get_stripe_payment_intent', [$this, 'get_stripe_payment_intent']);

            add_action('wp_ajax_process_subscription', [$this, 'process_subscription']);
            add_action('wp_ajax_nopriv_process_subscription', [$this, 'process_subscription']);

            add_action('wp_ajax_pprocess_subscription_providoor', [$this, 'process_subscription_providoor']);
            add_action('wp_ajax_nopriv_process_subscription_providoor', [$this, 'process_subscription_providoor']);

            /* Salesforce - Order complete */
            add_action('wp_ajax_order_complete_providoor', [$this, 'order_complete_providoor']);
            add_action('wp_ajax_nopriv_order_complete_providoor', [$this, 'order_complete_providoor']);

            add_action('wp_ajax_order_complete', [$this, 'order_complete']);
            add_action('wp_ajax_nopriv_order_complete', [$this, 'order_complete']);

            /* Coupons */
            add_action('admin_post_tbm_create_coupon', [$this, 'create_coupon']);
            add_action('admin_post_tbm_update_coupon', [$this, 'update_coupon']);

            /* Check Coupon */
            add_action('wp_ajax_check_coupon', [$this, 'ajax_check_coupon']);
            add_action('wp_ajax_nopriv_check_coupon', [$this, 'ajax_check_coupon']);
            ...
            add_action('wp_ajax_ajax_process_upcoming_renewals', [$this, 'ajax_process_upcoming_renewals']);
            add_action('wp_ajax_nopriv_process_upcoming_renewals', [$this, 'ajax_process_upcoming_renewals']);
        ...
        {
            wp_send_json_success($this->config['stripe']['publishable_key']);
            wp_die();
        }
        ...
        public function process_subscription_providoor()
        {
            ...
            wp_send_json_success(['subscription' => $subscription, 'invoice' => []]);
            wp_die();
        }
        ...
        public function process_subscription()
        {
            ...
            if ($post['is_gift'] == 'yes' && !is_email($post['sub_email_reciever'])) {
                wp_send_json_error(['error' => ['message' => 'Whoops, looks like you have entered invalid email address! - Gift email']]);
                wp_die();
            }
            ...
            $shipping = [
                'sub_full_name' => $post['sub_full_name'], // 'shipping_full_name
                'sub_email_reciever' => $post['sub_email_reciever'], // 'shipping_email_reciever
            ...
            if (isset($post['buy_option'])) {
                $price = $this->setPrice($post['buy_option']);

                if ($post['buy_option'] == 'printonly') {
                    $product_description = 'Rolling Stone Australia Magazine Subscription (4 issues)';
                } elseif ($post['buy_option'] == 'printdigital') {
                    $product_description = 'Rolling Stone Australia Magazine Subscription (4 issues) + Digital Access';
                } elseif ($post['buy_option'] == 'digitalonly') {
                    $product_description = 'Rolling Stone Australia Magazine Digital Access';
                }

                if ($price == 0) {
                    wp_send_json_error([
                        'error' => 'Please select a subscription type (Print, Print & Digital or Digital Only) before applying your coupon.',
                    ]);
                    wp_die();
                }
            } else {
            ...
            if ($invoice) {
                ...
                wp_send_json_success(['subscription' => $subscription, 'invoice' => $invoice['invoice']]);
                wp_die();
            } // If $customer
        }
...
<?php }?>
                                    ...
                                    <td>
                                            <a href="https://sales.thebrag.media/app/v1-subscription#id=<?php echo $sub->salesforce_id; ?>/view" target="_blank">
    ...
    <?php
            ...
            public function order_complete_providoor()
            {
                ...
                if (isset($crm_record->success) && $crm_record->success === false) {
                    $subscriber->update([
                        'crm_error' => true,
                    ]);

                    wp_send_json_error(['error' => ['message' => 'Error']]);
                    wp_die();
                } else {
                    $subscriber->update([
                        'salesforce_id' => $crm_record->data->Id,
                    ]);
                }
            ...
            public function order_complete()
            {
                ...
                /*
                 * Create digital subscription (if buy option is digital or print+digital)
                 */

                if ($subscriber->buy_option == 'digitalonly' || $subscriber->buy_option == 'printdigital') {
                    $digital = new Digital();
                    ...
                    if (isset($digital_subsciber['error'])) {
                        ...
                        $digital_subsciber_library = $digital->createSubscriptionLibrary($subscriber);
                    }
                }
    ...
    <?php
        include get_template_directory() . '/email-templates/footer.php';
            ...
            public function ajax_check_coupon()
            {
                ...
                if (isset($buy_option)) {
                    $price = $this->setPrice($buy_option);
                } else {
                    wp_send_json_error([
                        'amount_off' => 0,
                        'amount_to_pay' => $price + $this->shipping_cost,
                        'error' => 'Please select a subscription type (Print, Print & Digital or Digital Only) before applying your coupon.',
                    ]);
                    wp_die();
                }
            ...
            {
                register_rest_route($this->plugin_name . '/v1', '/get', [
                    'methods' => 'GET',
                    'callback' => [$this, 'rest_get_subscriptions'],
                    'permission_callback' => '__return_true',
                ]);

                register_rest_route($this->plugin_name . '/v1', '/update_shipping', [
                    'methods' => 'POST',
                    'callback' => [$this, 'rest_update_shipping'],
                    'permission_callback' => '__return_true',
                ]);

                register_rest_route($this->plugin_name . '/v1', '/update_billing', [
                    'methods' => 'POST',
                    'callback' => [$this, 'rest_update_billing'],
                    'permission_callback' => '__return_true',
                ]);

                register_rest_route($this->plugin_name . '/v1', '/cancel_autorenew', [
                    'methods' => 'POST',
                    'callback' => [$this, 'rest_cancel_autorenew'],
                    'permission_callback' => '__return_true',
                ]);

                register_rest_route($this->plugin_name . '/v1', '/cancel_subscription', [
                    'methods' => 'POST',
                    'callback' => [$this, 'rest_cancel_subscription'],
                    'permission_callback' => '__return_true',
                ]);

                register_rest_route($this->plugin_name . '/v1', '/enable_autorenew', [
                    'methods' => 'POST',
                    'callback' => [$this, 'rest_enable_autorenew'],
                    'permission_callback' => '__return_true',
                ]);

                register_rest_route($this->plugin_name . '/v1', '/update_customer_id', [
                    'methods' => 'POST',
                    'callback' => [$this, 'rest_update_customer_id'],
                    'permission_callback' => '__return_true',
                ]);

                register_rest_route($this->plugin_name . '/v1', '/updated_payment_details', [
                    'methods' => 'POST',
                    'callback' => [$this, 'rest_updated_payment_details'],
                    'permission_callback' => '__return_true',
                ]);

                register_rest_route($this->plugin_name . '/v1', '/st_h', [
                    'methods' => 'POST',
                    'callback' => [$this, 'rest_stripe_hook'],
                    'permission_callback' => '__return_true',
                ]);

                register_rest_route($this->plugin_name . '/v1', '/current_issue_img', [
                    'methods' => 'GET',
                    'callback' => function () {
                ...
                register_rest_route($this->plugin_name . '/v1', '/next_issue_img', [
                    'methods' => 'GET',
                    'callback' => function () {
                ...
                register_rest_route($this->plugin_name . '/v1', '/next_issue_img_thumb', [
                    'methods' => 'GET',
                    'callback' => function () {
                ...
                register_rest_route($this->plugin_name . '/v1', '/next_issue_details', [
                    'methods' => 'GET',
                    'callback' => [$this, 'rest_next_issue_details'],
                    'permission_callback' => '__return_true',
                ]);

                register_rest_route($this->plugin_name . '/v1', '/create_new', [
                    'methods' => 'POST',
                    'callback' => [$this, 'rest_create_new'],
                    'permission_callback' => '__return_true',
                ]);

                register_rest_route($this->plugin_name . '/v1', '/get_subscriber_emails', [
                    'methods' => 'GET',
                    'callback' => [$this, 'rest_get_subscriber_emails'],
                    'permission_callback' => '__return_true',
                ]);

                register_rest_route($this->plugin_name . '/v1', '/get_buy_options', [
                    'methods' => 'GET',
            ...
            {
                wp_send_json_success($this->config['magazine']);
                wp_die();
            }
...
<?php
    if (isset($_GET['manual'])) {
        ...
        public function ajax_process_upcoming_renewals()
        {
            require_once __DIR__ . '/classes/renewals.class.php';
            $renewals = new Renewals();
            return $renewals->ajax_process_upcoming_renewals();
        }
        ...
        {
            date_default_timezone_set('Australia/NSW');
            $next_run_timestamp = wp_next_scheduled('cron_hook_tbm_mag_sub_renewals', [null, null]);
