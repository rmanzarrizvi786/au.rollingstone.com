  ...
  public function process_subscription()
  {
    ...
    */
    if ($invoice) :
      ...
      endif; // If $this->stripe['current_coupon_code'] is NOT null
        ...
        <tbody id="the-list">
          <?php foreach ($subs as $sub) :
            ...
            <tr>
              ...
              <td><?php echo $sub->coupon_code; ?></td>
              <td><?php echo $sub->stripe_coupon_code; ?></td>
            ...
            <?php endif; ?>
              ...
              <td><a href="https://dashboard.stripe.com/customers/<?php echo $sub->stripe_customer_id; ?>" target="_blank"><?php echo $sub->stripe_customer_id; ?></a></td>
              <td><?php echo $sub->uniqid; ?></td>
            </tr>
          <?php endforeach; ?>
        </tbody>
      <?php endif; ?>
  ...
  */
  public function order_complete()
  {
    if (!check_ajax_referer($this->plugin_name . '_nonce', 'nonce')) :
      ...
      wp_mail('sachin.patel@thebrag.media', 'RS Mag Error: Invalid Nonce', 'Invalid Nonce');
      wp_send_json_error(['error' => ['message' => 'Whoops, like something unexpected happened on our side of things. Feel free to refresh your browser and give it another shot!']]);
      wp_die();
    endif;
    ...
    */
    ...
    if (!is_null($subscriber->coupon_id) && $subscriber->coupon_id > 0) :
      ...
      endif; // If coupon is found
    endif; // If subscriber has used coupon
    ...
    */
    ...
    // $headers[] = 'Bcc: sachin.patel@thebrag.media, daniel.boukata@thebrag.media';
    $to = $subscriber->email;
  ...
  ?>
    ...
    <?php
      endif; // If coupon found
    endif; // If Promotion is on
    ?>
      ...
      if ($subscription) {
        ...
        if ($crm_sub) {
          ...
          if (isset($crm_response['error'])) {
            ...
            wp_mail('sachin.patel@thebrag.media', 'Stripe Webhook: CRM updatePaymentAttempts', $crm_sub['Email__c'] . ' => ' . $crm_response['error']);
            wp_die();
          }
          ...
          if (isset($crm_response['error'])) {
            ...
            wp_mail('sachin.patel@thebrag.media', 'Stripe Webhook: CRM resetRemainingIssues', $crm_sub['Email__c'] . ' => ' . $crm_response['error']);
            wp_die();
          }
        } else {
          wp_mail('sachin.patel@thebrag.media', 'Stripe Webhook: CRM Record not found', print_r($invoice, true));
        }
      }
    }
  }
