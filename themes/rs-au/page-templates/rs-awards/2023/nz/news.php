<h2 class="text-center my-3 d-flex" style="font-family: GraphikXCondensed-BoldItalic; font-size: 250%;">
    <span class="mx-2">NEWS</span>
</h2>
<div class="d-flex flex-wrap align-items-start news-links">
    <div class="col-12 col-md-6 px-2">
        <a href="https://au.rollingstone.com/music/music-news/bic-runga-receives-icon-status-at-2023-panhead-rolling-stone-aotearoa-awards-50043/" target="_blank" class="d-flex my-2">
            <div class="img-wrap">
                <img src="https://images.thebrag.com/cdn-cgi/image/fit=crop,width=1200,height=628/https://images-r2.thebrag.com/rs/uploads/2020/09/bic-runga-1.jpg">
            </div>
            <h3>Bic Runga Receives Icon Status at 2023 Panhead Rolling Stone Aotearoa Awards</h3><br />
        </a>
    </div>

    <div class="col-12 col-md-6 px-2">
        <a href="https://au.rollingstone.com/music/music-news/2023-panhead-rolling-stone-aotearoa-awards-performers-bic-runga-coterie-49875/" target="_blank" class="d-flex my-2">
            <div class="img-wrap">
                <img src="https://images.thebrag.com/cdn-cgi/image/fit=crop,width=1200,height=628/https://images-r2.thebrag.com/rs/uploads/2023/09/bic-runga-1.jpg">
            </div>
            <h3>Bic Runga & Coterie to Perform at 2023 Panhead Rolling Stone Aotearoa Awards</h3><br />
        </a>
    </div>

    <div class="col-12 col-md-6 px-2">
        <a href="https://au.rollingstone.com/music/music-news/2023-rolling-stone-aotearoa-awards-nominees-revealed-six-60-the-beths-fazerdaze-48604/" target="_blank" class="d-flex my-2">
            <div class="img-wrap">
                <img src="https://images.thebrag.com/cdn-cgi/image/fit=crop,width=1200,height=628/https://images-r2.thebrag.com/rs/uploads/2022/08/six60-album-single.jpg">
            </div>
            <h3>Six60, The Beths & Fazerdaze Among 2023 Panhead Rolling Stone Aotearoa Awards Nominees</h3>
        </a>
    </div>

    <div class="col-12 col-md-6 px-2">
        <a href="https://au.rollingstone.com/music/music-news/panhead-rolling-stone-aotearoa-awards-2023-nominations-48376/" target="_blank" class="d-flex my-2">
            <div class="img-wrap">
                <img src="https://images.thebrag.com/cdn-cgi/image/fit=crop,width=1200,height=628/https://images-r2.thebrag.com/rs/uploads/2022/08/ROLLINGSTONE_AWARDS_AKL-30.jpg">
            </div>
            <h3>Last Chance to Nominate for the Panhead Rolling Stone Aotearoa Awards 2023!</h3><br />
        </a>
    </div>

    <div class="col-12 col-md-6 px-2">
        <a href="https://au.rollingstone.com/music/music-news/rolling-stone-aotearoa-awards-2023-nominate-now-47758/" target="_blank" class="d-flex my-2">
            <div class="img-wrap">
                <img src="https://images.thebrag.com/cdn-cgi/image/fit=crop,width=1200,height=628/https://images-r2.thebrag.com/rs/uploads/2022/08/ROLLINGSTONE_AWARDS_AKL-30.jpg">
            </div>
            <h3>Panhead Rolling Stone Aotearoa Awards 2023: Nominate Now!</h3><br />
        </a>
    </div>
</div>

<h2 class="text-center my-2 d-flex" style="font-family: GraphikXCondensed-BoldItalic; font-size: 250%;">
    <img src="<?php echo get_template_directory_uri(); ?>/images/rsa2022/nz/arrowE60000.svg" width="32">
    <span class="mx-2">ROLLING STONE NZ AWARDS 2022 WRAP-UP</span>
    <img src="<?php echo get_template_directory_uri(); ?>/images/rsa2022/nz/arrowE60000.svg" width="32" style="transform: rotate(180deg);">
</h2>
<div class="d-flex flex-column my-4">
    <div class="text-center" style="width: 890px; max-width: 100%;font-family: Graphik-Medium, sans-serif;">
            <div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;" itemscope itemtype="https://schema.org/VideoObject"><meta itemprop="thumbnailUrl" content="https://s2.dmcdn.net/v/UCG2-1Z6kuQZSbuLX/x120" /><meta itemprop="duration" content="P87S" /><meta itemprop="embedUrl" content="https://www.dailymotion.com/embed/video/x8dknge" /><iframe style="width:100%;height:100%;position:absolute;left:0px;top:0px;overflow:hidden" frameborder="0" type="text/html" src="https://www.dailymotion.com/embed/video/x8dknge" width="100%" height="100%"   allow="autoplay" allowfullscreen></iframe></div>
</div>
