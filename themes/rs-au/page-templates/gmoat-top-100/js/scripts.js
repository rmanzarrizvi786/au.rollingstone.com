...
jQuery(document).ready(function($) {
  ...
  $(document).on('submit', '#gmoat-vote-form', function(e) {
    e.preventDefault();

    var theForm = $(this);
    var formData = new FormData( theForm[0] );
    var select_movies = [];
    formData.append( 'action', 'save_gmoat_vote' );

    // formData.append( 'movies', theForm.find('#select-movies').val() );
    for( var i = 1; i <= 3; i++ ) {
      var m = theForm.find('#select-movies' + i).data('id');
      if ( m )
        select_movies.push( m );
    }
    formData.append( 'movies', select_movies );
    ...
    for( var i = 1; i <= 3; i++ ) {
      ...
      if ( '' != $.trim( elem.val() ) ) {
        $('#voted_movies').append( '<li><i class="fas fa-check"></i> ' + elem.val() + '</li>' );
      }
    }
    ...
    $.ajax({
      ...
      success: function(res, textStatus, jqXHR) {
        if ( res.success ) {
          $('#js-success').html( 'Thanks for voting in <em>Rolling Stone\'s 100 Great Movies Of All Time. If you would like, you can also submit a wildcard entry below and enter for the chance to win a Sony 65-inch 4K TV and 4K UHD Blu-ray player.' ).show();
          
          console.log( res );

          if ( res.data.entered_comp == true ) {
            $('#gmoat-comp-form').find('input,select,textarea').val(null);
            $('#gmoat-comp-form').find('.select-movies').val(null).trigger('change');
            $('#js-success').html( 'Your submission has been confirmed, thanks for entering!' ).show();
          ...
          theForm.slideUp();
          theForm.detach();
          $('#gmoat-comp-form-wrap').slideDown().find('#entry_id').val(res.data.id);

          if ( res.data.share_url != 'undefined' ) {
            $('#share_facebook').prop( 'href', 'https://www.facebook.com/sharer.php?u=' + res.data.share_url );
            $('#share_twitter').prop( 'href', 'https://twitter.com/intent/tweet?text=&url=' + res.data.share_url );
          }
  ...
  $(document).on('submit', '#gmoat-comp-form', function(e) {
    ...
    var theForm = $(this);
    var formData = new FormData( theForm[0] );
    formData.append( 'action', 'save_gmoat_comp' );
    formData.append( 'user_entry_movie', theForm.find('#user_entry_movie').val() );
...
document.addEventListener('DOMContentLoaded', function(){
  imagesLoaded( document.querySelector('#slideshow-wrap1'), function( instance ) {
  ...
  imagesLoaded( document.querySelector('#slideshow-wrap2'), function( instance ) {
