 ...
 * @since 2018-04-13
 */
class Social_Header_Nav_Walker extends \Rolling_Stone\Inc\Menus\Social_Nav_Walker {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$output .= sprintf(
			'<li class="c-social-bar__item"><a href="%s" class="c-social-bar__link c-social-bar-header__link" title="%s" rel="noopener noreferrer"%s><span class="c-icon c-icon--red c-icon--large"><svg><use xlink:href="#svg-icon-%s"></use></svg></span><span class="screen-reader-text">%s</span></a>',
			esc_url( $item->url ),
			esc_attr( $item->title ),
			$target,
			esc_attr( $icon ),
