...
class Lists
{
	...
	protected function __construct()
	{
		...
		add_filter('views_edit-' . self::LIST_ITEM_POST_TYPE, [$this, 'show_current_list'], 99);
	...
	public function admin_enqueue_scripts()
	{
		if (is_admin() && self::LIST_ITEM_POST_TYPE === get_post_type()) {
			wp_enqueue_script('pmc-lists', TBM_CDN . '/assets/js/list.js', ['jquery'], '2.0', true);
	...
	public function get_lists()
	{
		...
		$nonce = \PMC::filter_input(INPUT_POST, 'nonce');

		if (empty($nonce) || !wp_verify_nonce($nonce, 'pmc-lists')) {
			wp_send_json_error('Nonce verification failed.');
		}

		$search_query = \PMC::filter_input(INPUT_POST, 'query');
	...
	public function list_item_add_custom_column($default)
	{
		...
		$default['order'] = __('Order', 'pmc-lists');
	...
	public function list_item_manage_custom_column($column_name, $list_item_id)
	{
		...
		if (!empty($column_name) && !empty($list_item_id) && 'order' === $column_name) {
			$list_item = get_post($list_item_id);
			echo $list_item->menu_order;
		}
	}
