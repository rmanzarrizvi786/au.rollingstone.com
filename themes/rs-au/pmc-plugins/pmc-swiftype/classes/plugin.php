...
class Plugin {
	...
	public function is_enabled( $opt = 'pmc_swiftype' ) {
		...
		return true; // 'yes' === PMC_Cheezcap::get_instance()->get_option( $opt );
	}
	...
	public function get_default_sort_field() {
		$PMC_Cheezcap = new PMC_Cheezcap();
		// $sort_type = PMC_Cheezcap::get_instance()->get_option( 'pmc_swiftype_sort_field' );
		$sort_type = $PMC_Cheezcap->get_option( 'pmc_swiftype_sort_field' );
	...
	public function get_settings() {
		 ...
		 * - meta_tags - Includes post_types, tags, topics, and appeared_in_print.
		 *
		 * @since 2015-11-19 - Mike Auteri - PPT-6376
		 * @version 2015-11-19 - Mike Auteri - PPT-6376
		 */
	...
	public function add_meta_tags() {
		...
		if ( ! empty( $swiftype['meta_tags']['show_content_type_meta_tag'] )
		...
		printf( '<meta class="swiftype" name="body" data-type="text" content="%s" />', esc_attr( wp_strip_all_tags( strip_shortcodes( $post->post_content ) ) ) );
