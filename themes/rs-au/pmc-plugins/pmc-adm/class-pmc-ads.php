...
class PMC_Ads extends PMC_Singleton {
	...
	protected function _init(){
		...
		add_action( 'wp_head', [ $this, 'add_ad_rizer_js_tag' ] );
	...
	/**
	 * Check if the current post is on a "no ads" blacklist. If so, force PMC_Ads::get_ads_to_render() to find no ads.
	...
	public function action_wp_enqueue_scripts() {
		if ( ! is_admin() ) {
			...
			wp_enqueue_script( 'pmc-adm-loader', plugins_url( sprintf( 'js/loader%s', $js_extension ), __FILE__ ), $adm_loader_deps, '5.0' );
			...
			wp_enqueue_style( 'pmc-adm-style', plugins_url( 'css/pmc-adm-style.css', __FILE__ ) );
	...
	public function get_ads_to_render( $ad_location, $ad_title = '', $provider = '' ) {
		...
		foreach ( $ads as $ad ) {
			...
			$ad = array_merge( array( 'width' => 0, 'height' => 0, 'priority' => 10, 'slot-type' => '' ), $ad );
	 ...
	 * @param string $key
	 *
	 * @return PMC_Ad_Provider if found, otherwise false
	 */
	public function get_provider( $key ) {
		static $default_provider = null;
		if ( empty( $this->_providers[ $key ] ) ) {
			if ( empty( $default_provider ) ) {
				$default_provider = new \PMC\Adm\Providers\PMC_Default( $key );
			}
	...
	*/
	...
	/**
	 * Add ADRIZER script tag
	 *
	 * @return void
	 */
	public function add_ad_rizer_js_tag() : void {

		$ad_rizer_domain = apply_filters( 'pmc_adm_ad_rizer_domain', '' );

		if ( ! empty( $ad_rizer_domain ) ) {

			PMC::render_template( PMC_ADM_DIR . '/templates/ad-rizer-tag.php', [ 'domain' => $ad_rizer_domain ], true );
