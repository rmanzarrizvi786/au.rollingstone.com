...
function pmc_ads_interruptus_single() {
	...
	for ( var i = 0; i < endpoints.length; i++ ) {
		...
		if ( '' == pmc.cookie.get( 'pmc_interrupted_url', '' ) ) {
			// track browser with no cookie
			var tracking_img = new Image();
			tracking_img.src = <?php echo esc_js( \PMC::ssl_friendly_url( 'http://heartbeat.pmc.com/' ) ); ?> + 'track?host=' + document.location.host + '&nocookie=interstial';
