...
class PMC_Carousel { // extends PMC_Singleton {
	...
	?>
		<div class="wrap">
			...
			<?php if( isset( $_GET[ 'settings-updated' ] ) && $_GET[ 'settings-updated' ] == 'true' ): ?>
				<div id="message" class="updated"><p><?php esc_html_e( 'Settings saved.', 'pmc-carousel' ); ?></p></div><!-- .updated#message -->
			<?php endif; ?>
	...
	public function render( $taxonomy, $term, $num_articles = 5, $size='', $opts = array() ) {
		...
		$current_time_utc = date('Y-m-d'); //PMC_TimeMachine::create( 'UTC' )->format_as( 'U' );	//current timestamp per UTC
	...
	public function category_meta_box( $post = '' ) {
		...
		if ( ! empty( $meta_tax ) ) {
			$terms = get_terms( $meta_tax, 'hide_empty=0' );
		} else {
			$terms = get_terms( $taxonomies[0], 'hide_empty=0' );
		}
	?>
		...
		if ( isset( $_POST['carousel_cats_nonce'] ) && wp_verify_nonce( $_POST['carousel_cats_nonce'], basename( __DIR__ ) ) ) {
			...
			if ( isset( $_POST['pmc-carousel-override'] ) && '1' == $_POST['pmc-carousel-override'] ) {
				...
				if ( isset( $_POST['pmc-carousel-override-url'] ) && '' !== trim( $_POST['pmc-carousel-override-url'] ) ) {
					...
					$linked_post_id = wpcom_vip_url_to_postid( esc_url_raw( $_POST['pmc-carousel-override-url'] ) );
	...
	public function action_pmc_linkcontent_before_insert_field() {
		...
		/**
		 * this check will be removed once the tvline and variety themes deploy.
		 */
		if( !defined( 'USE_SHARED_CAROUSEL_PLUGIN') ){
			return;
		}

		$override_enabled = get_post_meta( get_the_ID(), '_pmc_carousel_override', true );
		$override_url = get_post_meta( get_the_ID(), '_pmc_carousel_override_url', true );
		?>
...
global $pmc_carousel;
$pmc_carousel = new PMC_Carousel(); // PMC_Carousel::get_instance();
...
function pmc_render_carousel( $taxonomy, $term, $num_articles = 5, $size='', $opts = false ) {
	$PMC_Carousel = new PMC_Carousel();
	return $PMC_Carousel->render( $taxonomy, $term, $num_articles, $size, $opts );
