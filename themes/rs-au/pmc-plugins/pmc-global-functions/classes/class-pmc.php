...
class PMC {
	 ...
	 *
	 * Note, this function caches it's result in static variables,
	 * then calls itself to return the 'cache'.
	 *
	 * @param string $kind   The kind of device to check for.
	 *                       Possible values: 'any, 'smart', or 'dumb'.
	 *
	 * @param string $caller Internal helper used to know when the function
	 *                       is calling itself, and in-turn for assisting in
	 *                       clearing the static variable cache (for unit testing).
	 *
	 * @since 2014-06-03 Corey Gilmore
	 */
	public static function is_mobile( $kind = 'any', $caller = '' ) {
		 ...
		 *
		 * Ignoring code coverage since our test suite is not set up to
		 * test changing PHP constant state for testing different cases.
		 */
		...
		if (
			...
			$mock_env = apply_filters( 'pmc_is_mobile_mock_env', false );
		...
		static $is_smart = null;
		static $is_dumb = null;

		// Filter and return false to reset the static variables.
		// This is helpful for unit tests which set $_SERVER['HTTP_USER_AGENT'] to spoof mobile.
		if ( false === apply_filters( 'pmc_is_mobile_use_static_variable_cache', true ) && 'self' !== $caller ) {
			$is_mobile = null;
			$is_smart  = null;
			$is_dumb   = null;
		}
		...
		if ( defined( 'PMC_IS_VIP_GO_SITE' ) && PMC_IS_VIP_GO_SITE ) {
			if ( isset( $_SERVER['HTTP_X_MOBILE_CLASS'] ) ) {
				$is_smart = 'smart' === $_SERVER['HTTP_X_MOBILE_CLASS'];
				$is_dumb = 'dumb' === $_SERVER['HTTP_X_MOBILE_CLASS'];
				$is_mobile = $is_smart || $is_dumb;
				return self::is_mobile( $kind, 'self' );
			}
		}
		...
		if( $is_mobile === null ) {
			...
			$is_mobile = jetpack_is_mobile( 'any', false ) ? true : false; // ensure we never have null
			$is_smart = jetpack_is_mobile( 'smart', false ) ? true : false; // ensure we never have null
			$is_dumb = jetpack_is_mobile( 'dumb', false ) ? true : false; // ensure we never have null
			return self::is_mobile( $kind, 'self' );
		}
	}
	...
	public static function strip_disallowed_urls( $content ) {
		 ...
		 * - external_url_whitelist_boolean boolean
		 * - external_strip_all_boolean boolean
		 * - site_hosts array
		 * - feeds_external_url_whitelist array
		 * - external_url_whitelist array
		 * - bypass_url_whitelist boolean
		 * @var $post_id integer
		 *
		...
		$args = array(
			...
			'external_url_whitelist_boolean' => false,
		...
		/**
		 * external_url_whitelist_boolean - If TRUE strip all non-whitelisted, non-external domains.
		 *
		 * @since 2015-08-12 - Mike Auteri - PPT-5181
		 * @version 2015-08-12
		 */
		$external_whitelist = $settings['external_url_whitelist_boolean'];
		...
		/**
		 * bypass_url_whitelist - If TRUE bypasses running content through this function.
		 *
		 * @since 2015-08-12 - Mike Auteri - PPT-5181
		 * @version 2015-08-12
		 */
		...
		if( $external_whitelist ) {
			if( is_feed() ) {
				 ...
				 * @version 2015-08-12
				 */
				$allowed_hosts = $settings['feeds_external_url_whitelist'];
			} else {

				/**
				 * external_url_whitelist - Array of external domain to whitelist in a non-feed.
				 *
				 * @since 2015-08-12 - Mike Auteri - PPT-5181
				 * @version 2015-08-12
				 */
				$allowed_hosts = $settings['external_url_whitelist'];
			}
		}
