...
<head>
    ...
    background: linear-gradient(-45deg, #d32531 50%, rgba(211, 37, 49, 0.5) 50%); */
		...
		.rsa-header-news {
			...
			position: relative;
			background-image: url(<?php echo get_template_directory_uri(); ?>/images/sj-road-to-rs/RoadToAwards_road.jpg); */
		}
	...
	<script>
		jQuery(document).ready(function($) {
			...
			/* $(window).on('scroll', function() {
				if (
					$(window).scrollTop() >=
					$('header').outerHeight() + $('.l-header__wrap.tbm').outerHeight()
				) {
					$('.l-header__wrap.tbm').addClass('scrolled');
				} else {
					$('.l-header__wrap.tbm').removeClass('scrolled');
				}
			});
...
&noscript=1" />
		...
		<div class="l-page" id="site_wrap" style="width: 100%; max-width: 100%; overflow-x: hidden">
			<?php get_template_part('template-parts/header/header', 'sj-road-to-rs'); ?>