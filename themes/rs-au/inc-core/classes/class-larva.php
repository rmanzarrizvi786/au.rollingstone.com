...
class Larva {
	...
	/**
	 * Get data structure from JSON File
	 *
	 * @param      $pattern_name
	 *                          ex:
	...
	public function get_json( $pattern_name, $from_parent_theme = false ) {
		...
		$path = ( true === $from_parent_theme ) ? PMC_CORE_PATH : CHILD_THEME_PATH;

		$pattern_name = apply_filters( 'pmc_core_larva_json_pattern_name', $pattern_name );

		$file_path = sprintf( '%s/assets/build/json/' . $pattern_name . '.json', untrailingslashit( $path ) );
