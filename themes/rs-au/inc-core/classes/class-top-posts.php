...
class Top_Posts {
	...
	protected static function get_random_posts( $period ) {
		...
		$query = new \WP_Query( [
			...
			'date_query'          => [
				[
					'after' => strtotime( '-' . $period . ' days', time() ),
				],
			],
			'meta_query'          => [
				[
	...
	public static function get_posts(
		...
		// 	$trending_post_ids[] = get_option( 'force_most_viewed' );
		// endif;
		$trending_post_ids = array_merge( $trending_post_ids, wp_list_pluck( $trending_posts, 'post_id' ) );
		
		// echo '<pre>'; print_r( $trending_post_ids ); exit;
		...
		if(is_single()) {
			...
			if(isset($brand_category) && $brand_category != "") {
		...
		$cache_key = 'pmc-most-popular-' . md5( $limit . $days . $period . $type . $section . $brand_category . $cache_duration );
