...
class Byline {
	...
	**/
		...
		if ( ! empty( $authors ) && is_array( $authors ) ) {
			...
			foreach ( $authors as $k => $v ) {
				...
				if ( 'guest-author' === $authors[$k]->type ) {
					$link = '<a href="%s" itemprop="author" itemscope itemtype="http://schema.org/Person"><span>%s</span></a>';
				} else {
					$link = '<a href="%s" itemprop="contributor" itemscope itemtype="http://schema.org/Person">%s</a>';
				}
				$output.= sprintf(
					$link,
	...
	function tbm_get_authors( $post_id = 0 ) {
