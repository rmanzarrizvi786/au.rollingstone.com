	...
	<div class="c-byline__authors">
		...
		if (!empty($authors) && is_array($authors)) {
				...
				<div class="c-byline__author">
					<?php if ($author_count > 1) { ?>
						<a href="<?php echo esc_url(get_author_posts_url($author->ID, $author->user_nicename)); ?>"
							class="c-byline__link t-bold t-bold--upper" rel="author noopener noreferrer external">
					...
					<?php } else {
						...
						<a href="<?php echo esc_url(get_author_posts_url($author->ID, $author->user_nicename)); ?>"
							...
							data-author="<?php echo $author->display_name; ?>">
							<?php echo esc_html($author->display_name); ?>
			...
			if (is_array($authors)) {
				...
				echo '<span class="c-byline__amp t-bold author" data-author="' . $authorsString . '">' . $authorsString . '</span>';
